...
/**
 * Ищет в объекте свойство и увеличивает его значение на единицу
...
function collectDOMStat(root) {
    ...
    elements.push(...root.childNodes);

    while (elements.length) {
        // берем элемент из списка для обработки
    ...
    var notify = (records) => {
        records.forEach(r => {
            if (r.addedNodes.length) {
                fn({ type: 'insert', nodes: [...r.addedNodes] });
            }
            if (r.removedNodes.length) {
                fn({ type: 'remove', nodes: [...r.removedNodes] });
            }
        });
    }

    var mo = new MutationObserver(notify);

    mo.observe(where, { childList: true, subtree: true });
}
